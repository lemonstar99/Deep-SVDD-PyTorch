...
class SAD_LeNet_Autoencoder(BaseNet):
    ...
    def __init__(self):
        ...
        self.deconv4 = nn.ConvTranspose2d(32, 93, 2, bias=False, padding=2)
        nn.init.xavier_uniform_(self.deconv4.weight, gain=nn.init.calculate_gain('leaky_relu'))

    def forward(self, x):
        print("00: ", x.size()) # [200, 93, 13]
        x = x.unsqueeze(3)
        print("01: ", x.size()) # [200, 93, 13, 1]
        x = self.conv1(x)
        print("02: ", x.size()) # [200, 32, 15, 3]
        x = self.pool(F.leaky_relu(self.bn2d1(x)))
        print("03: ", x.size()) # [200, 32, 7, 1]
        x = self.conv2(x)
        print("04: ", x.size()) # [200, 64, 8, 4]
        x = self.pool(F.leaky_relu(self.bn2d2(x)))
        print("05: ", x.size()) # [200, 64, 4, 1]
        x = self.conv3(x)
        print("06: ", x.size()) # [200, 128, 5, 2]
        x = self.pool(F.leaky_relu(self.bn2d3(x)))
        ...
        x = self.deconv4(x)
        print("18: ", x.size()) # [200, 93, 3, 3]
        x = torch.sigmoid(x)
        print("19: ", x.size()) # [200, 93, 3, 3]
        return x[:,:,:,0]