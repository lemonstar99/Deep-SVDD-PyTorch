...
class CT_Dataset(TorchvisionDataset):
    ...
    def __init__(self, root: str, normal_class=0):
        ...
        train_set = TensorDataset(torch.Tensor(np.array(x[test_count:])), torch.Tensor(np.array(y[test_count:])), torch.Tensor(np.array(285, 2858)))
        test_set = TensorDataset(torch.Tensor(np.array(x[:test_count])), torch.Tensor(np.array(y[:test_count])), torch.Tensor(np.array(0, 285)))        
