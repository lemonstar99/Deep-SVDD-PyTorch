...
class CT_Dataset(TorchvisionDataset):
    ...
    def __init__(self, root: str, normal_class=2):
        ...
        y_train_new = get_target_label_idx(torch.Tensor(np.array(y[test_count:])), self.normal_classes)
        y_test_new = get_target_label_idx(torch.Tensor(np.array(y[:test_count])), self.normal_classes)
        ...
        train_set = TensorDataset(torch.Tensor(np.array(x_train_transformed)), torch.Tensor(np.array(y_train_transformed)), torch.Tensor(np.arange(285, 2858)))
